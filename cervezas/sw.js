...
// "cervezas-" y el cleanup de "activate" solo toca cachés con ese prefijo,
// para no borrar nunca la caché de otra app del mismo hosting.
const CACHE_PREFIX = "cervezas-shell-";
...
self.addEventListener("fetch", (event) => {
  ...
  // Red primero, caché como respaldo solo si no hay conexión, así siempre
  // que hay señal se ve la última versión desplegada al instante.
  event.respondWith(
    fetch(request)
      .then((response) => {
