...
// "nombremates-" y el cleanup de "activate" solo toca cachés con ese prefijo,
// para no borrar nunca la caché de otra app del mismo hosting.
const CACHE_PREFIX = "nombremates-shell-";
...
self.addEventListener("fetch", (event) => {
  const { request } = event;
  // Nunca cachear llamadas a Supabase, ni a los servicios RDAP externos:
  // siempre queremos datos frescos (disponibilidad de dominio en vivo).
  if (request.url.includes(".supabase.co")) return;
  if (request.url.includes("rdap")) return;
