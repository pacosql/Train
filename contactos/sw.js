...
// "contactos-" y el cleanup de "activate" solo toca cachés con ese prefijo,
// para no borrar nunca la caché de otra app del mismo hosting.
const CACHE_PREFIX = "contactos-shell-";
...
self.addEventListener("fetch", (event) => {
  ...
  // Red primero, caché como respaldo solo si no hay conexión: siempre que
  // hay señal se ve la última versión desplegada.
  event.respondWith(
    fetch(request)
      .then((response) => {
